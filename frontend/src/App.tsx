  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchCart();
        // Fetch stats to see the new discount code
      ...
      <header className="bg-blue-600 text-white p-4 shadow">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm">User: {userId.substring(0, 12)}...</span>
            <button
              onClick={handleViewStats}
              className="bg-white text-blue-600 px-3 py-1 rounded text-sm hover:bg-gray-100"
            >
              {showStats ? "Dashboard" : "Admin Stats"}
            </button>
          </div>
        </div>
      </header>
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-8">
            ...
            <div>
              <h3 className="text-lg font-semibold mb-3">
                Discount Codes ({stats.discountCodes.total})
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {stats.discountCodes.list.map((code: any, idx: number) => (
                  <div
                    key={idx}
